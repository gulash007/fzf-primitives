...
from .options import Options, POSITION, EMPTY_OPTS
...
def add_options(added_options: Options):
    def decorator(func):
        ...
        def adding_options(*args, options: Options = EMPTY_OPTS, **kwargs):
...
def exit_round_on_no_selection(func):
    def exiting_round_on_no_selection(*args, options: Options = EMPTY_OPTS, **kwargs):
...
def preview(
    ...
    def decorator(func):
        @functools.wraps(func)
        def with_preview(*args, options: Options = EMPTY_OPTS, **kwargs):
            # print(type(self))
...
def hotkey(hk: str, action: Callable | str):
    ...
    def decorator(func):
        @functools.wraps(func)
        def with_hotkey(*args, options: Options = EMPTY_OPTS, **kwargs):
...
def hotkey_python(hk: str, action: Callable):
    def deco(func):
        def with_python_hotkey(*args, options: Options = EMPTY_OPTS, **kwargs):
...
def clip_output(func):
    @functools.wraps(func)
    def clipping_output(*args, options: Options = EMPTY_OPTS, **kwargs):
